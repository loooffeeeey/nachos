...
class Semaphore {
  ...
  private:
    char* name;        // useful for debugging DEbug的时候写出的辨别用的标志
    int value;         // semaphore value, always >= 0  信号量的值
    List *queue;       // threads waiting in P() for the value to be > 0  等待序列，P的时候小于0，然后等待的线程
};
...
// the condition variable operations.//////一句话，互斥操作
//
