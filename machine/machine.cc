...
Machine::Machine(bool debug)
{
    ...
    for (i = 0; i < NumTotalRegs; i++)
        registers[i] = 0;
    mainMemory = new char[MemorySize];///////////////一个辣鸡数组用来模仿内存卧槽？
    for (i = 0; i < MemorySize; i++)
      	mainMemory[i] = 0;
#ifdef USE_TLB
    tlb = new TranslationEntry[TLBSize]; ///////////////////translate.h
    for (i = 0; i < TLBSize; i++)
	tlb[i].valid = FALSE;
    pageTable = NULL;
...
void
Machine::RaiseException(ExceptionType which, int badVAddr)////这个参数从哪传进来
{
...
void Machine::Debugger()
{
    ...
    fflush(stdout);
    fgets(buf, 80, stdin);
    if (sscanf(buf, "%d", &num) == 1)////////////意思是只从stdin中读出一个字符吗
	runUntilTime = num;
    else {
	runUntilTime = 0;
	switch (*buf) {
