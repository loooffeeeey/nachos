...
//	programs, and accesses to kernel memory are not translated or paged.（这里的两个动词不知道什么意思）
//
...
enum ExceptionType { NoException,           // Everything ok!
		     ...
		     ReadOnlyException,     // Write attempted to page marked 
					    // "read-only"
		     BusErrorException,     // Translation resulted in an （往无效的地址中传东西）（bus）
					    // invalid physical address
		     AddressErrorException, // Unaligned reference or one that（地址越界）（想想数组）
					    // was beyond the end of the
					    // address space
		     OverflowException,     // Integer overflow in add or sub.（溢出）（想想65536）
		     IllegalInstrException, // Unimplemented or reserved instr.（没有完成的命令）
		     
		     NumExceptionTypes   ///数字异常，可能除以0了吧
};
...
class Instruction {/////////////////////////////////////这个类好像其中的东西还没有实现
  public:
